...
if (isset($_REQUEST["addtoplaylist"])) {
  ...
  if ($mysql->insert("playlist",["Name"=>$_REQUEST["addplaylist"]])) {
    echo '<span><img class="delete" target="' . $mysql->insert_id  . '" src="pix/delete.png"/><a href="#'.$mysql->insert_id .'" class="showplaylist">' . $_REQUEST["addplaylist"] . '</a><form action="'.$base.'/showplaylist.php" method="POST" style="display:inline;margin-left:5px;position:relative;top:7px;"><input type="hidden" name="playlist" value="'.$mysql->insert_id .'"><input type="image" alt="Submit Form" style="width:25px;height:25px;" src="'.$base.'/pix/play.png"/></form><BR></span>';
  ...
  foreach ($result as $row) {
    echo '<span><img class="delete" target="' . $row["ID"] . '" src="pix/delete.png"/><a href="#'.$row["ID"].'" class="showplaylist">' . $row["Name"] . '</a><form action="'.$base.'/showplaylist.php" method="POST" style="display:inline;margin-left:5px;position:relative;top:7px;"><input type="hidden" name="playlist" value="'.$row["ID"] .'"><input type="image" alt="Submit Form" style="width:25px;height:25px;" src="'.$base.'/pix/play.png"/></form><BR></span>';
